...
def sft_red_teaming_evaluation(
    ...
    with accelerator.main_process_first():
        if (
            ...
            forget_train, adv_retain = dataloader_type(
    ...
    if (
        ...
        dataloaders = [adv_retain, forget_train]
    else:
    ...
    model = loop_type(
        model,
        tokenizer,
        dataloaders[0],    # retain_dataloader (adv_retain)
        dataloaders[1],    # forget_train_dataloader (forget_train)
        dataloaders[1],    # forget_test_dataloader (no separate test, reuse forget_train)
        optimizer,
        scheduler,
        accelerator,
...
def main():
    ...
    parser.add_argument("--tar_adversary_batch_size", "-advbs", type=int, default=2)
    parser.add_argument(
