...
def fsdp_v1_model_params(model: FSDP):
    """
    Get all model parameters via FSDP handles (compatible with PyTorch FSDP v1 and v2)
    """
    visited = set()

    # Try FSDP v1 path (older PyTorch)
    if hasattr(model, '_all_handles'):
        sharded_params = set()
        ...
        for _, handle in enumerate(model._all_handles):
            ...
            if id(handle.flat_param) not in visited:
                visited.add(id(handle.flat_param))
        ...
        for _, param in model.named_parameters():
            ...
            if not_fsdp_managed and id(param) not in visited:
                visited.add(id(param))
                yield param

    # Fallback: iterate all parameters directly (FSDP v2 / newer PyTorch)
    else:
        for _, param in model.named_parameters():
